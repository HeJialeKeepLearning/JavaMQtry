            ...
            try {
                for (int i = 0; i < topics.size(); i++) {
                    ...
                    for (int j = 0; j < PUSH_COUNT; j++) {
                        ...
                        msg.putHeaders(MessageHeader.MESSAGE_ID, 1);
                        msg.putHeaders(MessageHeader.BORN_TIMESTAMP, 1L);
                        msg.putHeaders(MessageHeader.BORN_HOST, "test1");
                        msg.putHeaders(MessageHeader.STORE_TIMESTAMP, 2L);
                        msg.putHeaders(MessageHeader.STORE_HOST, "test2");
                        msg.putHeaders(MessageHeader.START_TIME, 3L);
                        msg.putHeaders(MessageHeader.STOP_TIME, 4L);
                        msg.putHeaders(MessageHeader.TIMEOUT, 2);
                        msg.putHeaders(MessageHeader.PRIORITY, 3);
                        msg.putHeaders(MessageHeader.RELIABILITY, 4);
//                        msg.putHeaders(MessageHeader.SEARCH_KEY, "test3");
                        msg.putHeaders(MessageHeader.SCHEDULE_EXPRESSION, "test4");
                        msg.putHeaders(MessageHeader.SHARDING_KEY, 1.0);
                        msg.putHeaders(MessageHeader.SHARDING_PARTITION, 2.0);
                        msg.putHeaders(MessageHeader.TRACE_ID, "test3");
                        //发送消息
                        producer.send(msg);
                ...
                while (true) {
                    ...
                    if (msg == null) {
                        ...
                        if (!msg.headers().getString(MessageHeader.SEARCH_KEY).equals("hello")) {
                            ...
                            System.exit(0);
                        }if (msg.headers().getInt(MessageHeader.MESSAGE_ID)!=1) {
                            System.out.println(String.format("header错误 topic %s 序号:%d", topic, j));
                            System.exit(0);
                        }
                        if (msg.headers().getLong(MessageHeader.STORE_TIMESTAMP)!=2L) {
                            System.out.println(String.format("header错误 topic %s 序号:%d", topic, j));
                            System.exit(0);
                        }
